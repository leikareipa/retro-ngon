...
//
// Returns a Promise that resolves when the baking has finished, i.e. when the shade
// value of all vertices has been computed.
export function bake_vertex_lightmap(ngons = [Rngon.ngon()],
                                     lights = [Rngon.light()],
                                     epsilon = 0.00001)
{
    return new Promise((resolve)=>
    {
        ...
        for (const [i, triangle] of triangles.entries())
        {
                ...
                ((performance.now() - updateTimer) > 3000))
            {
                ...
                const mRemaining = Math.ceil(sRemaining / 60);
                const hRemaining = Math.ceil(mRemaining / 60);
                ...
                console.log(`Baking to vertices... ${Math.floor(percentDone)}%, ETA = ${timeLabel}`);
        ...
        // Remove the temporary normal data we appended to vertices.
        for (const triangle of triangles)
        {
            for (const vertex of triangle.vertices)
            {
