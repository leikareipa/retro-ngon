...
| *function*            | ngonFill                 | A function called by the renderer to rasterize the n-gons that were passed to **render()**. For more info, including the list of parameters, see the default function. Defaults to *null*, which is an alias for the default function, *Rngon.rasterize_ngon_cache*. |
...
// rasterizer, Rngon.rasterize_ngon_cache().
//
...
| *boolean*            | allowAlphaBlend   | If *true*, the 'color' property's alpha value will determine the extent to which this n-gon is blended into its background during rasterization. If *false*, no blending will be performed (same as with an alpha value of 255). Defaults to *true*. |
| *boolean*            | allowAlphaReject  | If *true*, any texel whose alpha value is not equal to 255 will be discarded during rasterization. Otherwise, texel alpha is ignored. If the n-gon is untextured, this setting has no effect. Defaults to *true*. |
