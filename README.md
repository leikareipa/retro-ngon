...
| *string*              | depthSort                | Type of depth sorting to use when ordering n-gons for rasterization. Possible values: "none" (no depth sorting; n-gons will be rendered in the order they were given), "painter" (painter's algorithm; n-gons furthest from the camera will be rendered first), "painter-reverse" (n-gons closest to the camera will be rendered first). As the renderer also uses depth buffering regardless of the sorting mode, "painter-reverse" allows for early rejection of occluded pixels and so may offer the best performance - although "none" saves the initial cost of sorting. Defaults to *"painter-reverse"*. |
